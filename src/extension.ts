...
async function pressed_escape() {
  ...
  if (editor) {
    ...
    if (
      state &&
      (state.get_mode() === Mode.Diff || state.get_mode() === Mode.DiffWait)
    ) {
...
async function login_clicked() {
  ...
  await vscode.env.openExternal(
    vscode.Uri.parse(
      `https://refact.smallcloud.ai/authentication?token=${global.streamlined_login_ticket}&utm_source=plugin&utm_medium=vscode&utm_campaign=login`
    )
  );
  let i = 0;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let disposable1 = vscode.commands.registerCommand(
    "refactaicmd.inlineAccepted",
    inline_accepted
  );
  let disposable2 = vscode.commands.registerCommand(
    "refactaicmd.codeLensClicked",
    code_lens_clicked
  );
  ...
  let disposable3 = vscode.commands.registerCommand(
    "refactaicmd.activateToolbox",
    f1_pressed
  );
  let disposable9 = vscode.commands.registerCommand(
    "refactaicmd.addPrivacyOverride0",
    (uri: vscode.Uri) => {
      if (!uri || !uri.fsPath) {
        return;
      }
  ...
  let disposable10 = vscode.commands.registerCommand(
    "refactaicmd.addPrivacyOverride1",
    (uri: vscode.Uri) => {
      if (!uri || !uri.fsPath) {
        return;
      }
  ...
  let disposable11 = vscode.commands.registerCommand(
    "refactaicmd.addPrivacyOverride2",
    (uri: vscode.Uri) => {
      if (!uri || !uri.fsPath) {
        return;
      }
  ...
  let disposable12 = vscode.commands.registerCommand(
    "refactaicmd.privacySettings",
    () => {
      PrivacySettings.render(context);
    }
  );
  let disposable13 = vscode.commands.registerCommand(
    "refactaicmd.completionManual",
    async () => {
      await vscode.commands.executeCommand(
        "editor.action.inlineSuggest.trigger"
      );
  ...
  let logout = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      await vscode.workspace
        .getConfiguration()
        .update("codify.apiKey", undefined, vscode.ConfigurationTarget.Global);
      fill_no_user();
    }
  );
...
export async function follow_intent_highlight(
  intent: string,
  ...
  await highlight.query_highlight(
    editor,
    intent,
    function_name,
    model_name,
...
export async function follow_intent_diff(
  intent: string,
  function_name: string,
  model_name: string,
  third_party: boolean
  ...
  if (
    ...
    let end_pos_in_chars = editor.document.lineAt(selection.end.line - 1).range
      .end.character;
  ...
  estate.save_intent(intent);
  await interactiveDiff.query_diff(
    editor,
    selection,
    function_name || "diff-selection",
    model_name,
    third_party
  );
}
...
export async function status_bar_clicked() {
  ...
  if (!editor) {
    ...
    let pause_completion = vscode.workspace
      .getConfiguration()
      .get("refactai.pauseCompletion");
    let buttons: string[] = [];
    if (access_level > 0) {
  ...
  if (selection === "Pause Completion") {
    vscode.workspace
      .getConfiguration()
      .update("refactai.pauseCompletion", true, true);
  } else if (selection === "Resume Completion") {
    vscode.workspace
      .getConfiguration()
      .update("refactai.pauseCompletion", false, true);
