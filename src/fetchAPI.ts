...
export async function ast_index_file(file_url: vscode.Uri) {
    // note: this doesn't work with new files
    const url = rust_url("/v1/ast-index-file");
    if(!url) {
        return Promise.reject(
			"/ast-index-file no rust binary working, very strange"
		);
    }
    ...
    const request = new fetchH2.Request(url, {
        ...
        cache: "no-cache",
        referrer: "no-referrer",
        body: JSON.stringify({ file_url: file_url.toString() }),
    });

    console.log(["ast-index-file", file_url.toString()]);

    const response = await fetchH2.fetch(request);

    if(response.ok === false) {
        console.log([`${url} http status`, response.status, response.statusText]);
        return Promise.reject(`ast-index-file bad status ${response.status}:[${response.statusText}]`);
    }



    console.log(["/ast-index-file success", response]);

    return maybe_show_ast_status();
}

export async function ast_force_reindex() {
    const url = rust_url("/v1/ast-force-reindex");
    if(!url) {
        return Promise.reject("/ast-force-reindex no rust binary working, very strange");
    }

    console.log("ast force reindex");
    const request = new fetchH2.Request(url, {
        method: "GET",
        redirect: "follow",
        cache: "no-cache",
    ...
    if(response.ok === false) {
        console.log([`${url} http status`, response.status, response.statusText]);
        return Promise.reject(`ast-force-reindex bad status ${response.status}:[${response.statusText}]`);
    }
