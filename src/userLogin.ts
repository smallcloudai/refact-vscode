...
export async function account_message(
  info: string,
  action: string,
  url: string
) {
...
export function check_if_login_worked() {
  ...
  if (!global.user_logged_in || !apiKey) {
    return false;
  }
  return true;
}
...
export function secret_api_key(): string {
  ...
  let manual_infurl = vscode.workspace
    .getConfiguration()
    .get("refactai.infurl");
...
export async function login() {
    ...
    try {
      ...
      if (json.retcode === "OK") {
        ...
        await vscode.workspace
          .getConfiguration()
          .update("refactai.apiKey", apiKey, vscode.ConfigurationTarget.Global);
        await usageStats.report_success_or_failure(
          true,
          "recall",
          recall_url,
          "",
        ...
        json.retcode === "FAILED" &&
        json.human_readable_message.includes("The API key") &&
  ...
  let client_version =
    vscode.extensions.getExtension("smallcloud.codify")!.packageJSON.version;
  ...
  try {
    ...
    if (json.retcode === "OK") {
      ...
      if (json.login_message) {
        await usabilityHints.show_message_from_server(
          "LoginServer",
          json.login_message
        );
      }
      ...
      json.retcode === "FAILED" &&
      json.human_readable_message.includes("rate limit")
    ) {
      await usageStats.report_success_or_failure(
        false,
        "login-failed",
        login_url,
        json.human_readable_message,
        ""
      );
      return "";
...
export async function inference_login(): Promise<boolean> {
  ...
  if (global.last_positive_result + 600 * 1000 < Date.now()) {
    // session (socket) dies by itself after some time
    console.log(
      "inference_login: last_positive_result too old, force disconnect"
    );
    await fetchH2.disconnectAll();
  }
  let manual_infurl = vscode.workspace
